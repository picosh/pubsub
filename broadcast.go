...
type PubSubBroadcast struct {
	Logger   *slog.Logger
	Channels *syncmap.Map[string, *Channel]
}

func (b *PubSubBroadcast) Cleanup() {
	toRemove := []string{}
...
func (b *PubSubBroadcast) GetChannels(channelPrefix string) []*Channel {
	var chans []*Channel
	b.Channels.Range(func(I string, J *Channel) bool {
...
func (b *PubSubBroadcast) GetChannel(channel string) *Channel {
	channelData, _ := b.Channels.Load(channel)
	return channelData
}

func (b *PubSubBroadcast) GetPubs(channel string) []*Pub {
	var pubs []*Pub
	b.Channels.Range(func(I string, J *Channel) bool {
		found := channel == I
		if found || channel == "*" {
...
func (b *PubSubBroadcast) GetSubs(channel string) []*Sub {
	var subs []*Sub
	b.Channels.Range(func(I string, J *Channel) bool {
		found := channel == I
		if found || channel == "*" {
...
func (b *PubSubBroadcast) ensure(channel string) *Channel {
...
func (b *PubSubBroadcast) Sub(channel string, sub *Sub) error {
...
func (b *PubSubBroadcast) Pub(channel string, pub *Pub) error {
	dataChannel := b.ensure(channel)
